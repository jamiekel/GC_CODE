...
!
      ...
      HET(ind_O3 ,  1) = HETO3(RH, TEMPK, SPC_O3, IO) !http://pubs.acs.org/doi/pdf/10.1021/jp075300i
      !--------------------------------------------------------------------
...
! !IROUTINE: HetO3
!
! !DESCRIPTION: Set the heterogenous chemistry rate for PAH.
!\\
!\\
! !INTERFACE:
!   
    FUNCTION HETO3(RH, T, TCO3, Input_Opt) RESULT( HET_O3 )
!
! !INPUT PARAMETERS: 
!
      REAL(fp), INTENT(IN)          :: RH, T, TCO3
	  TYPE(OptInput), INTENT(IN)    :: Input_Opt   ! Input options
!
! !RETURN VALUE:
! 
      REAL(fp)             :: HET_O3, KMAX, KO3
      REAL(fp)             :: A, B
...
!
	  ...
	  KAHAN_HET_OFF = Input_Opt%KAHAN_HET_OFF
	  
	  IF (KAHAN_HET_OFF) THEN
        ! Langmuir hinshelwood mechanism
        KMAX = 1.0E-2_fp
	    KO3  = 280.0E-15_fp
	    HET_O3 = Input_Opt%PAH_KMAX*((Input_Opt%PAH_KO3*TCO3)/(1+Input_Opt%PAH_KO3*TCO3))
	  ENDIF
	  
	  IF (.NOT. KAHAN_HET_OFF) THEN
	    ! Kahan 2016 approach slightly different the more common langmuir hinshelwood mechanism
	    ! approach above. This apporach is not wired in but can be set by uncommenting the values bellow
	    ! This is the apporach used by the POPs model.
	    ! k(obs) = A x [O3(g)] / (B + [O3(g)])
        ! for BaP: A = 5.5 x 10^-3 s^-1, B = 2.8 x 10^15 molec/cm3
	    A = 5.5E-3_fp
	    B = 2.8E15_fp
	    HET_O3 = (A*TCO3)/(B+TCO3)
	  ENDIF
